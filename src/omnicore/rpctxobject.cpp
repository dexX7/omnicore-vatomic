...
bool showRefForTx(uint32_t txType)
{
    switch (txType) {
        ...
        case OMNI_TYPE_SEND_UNIQUE: return true;
    }
...
void populateRPCTypeSendUnique(CMPTransaction& omniObj, Object& txobj)
{
    uint32_t propertyId = omniObj.getProperty();
    txobj.push_back(Pair("propertyid", (uint64_t)propertyId));
...
void populateRPCExtendedTypeGrantUnique(const uint256& txid, Object& txobj)
{
    LOCK(cs_tally);
