...
// omni_uniquesend - unique send
Value omni_sendunique(const Array& params, bool fHelp)
{
    ...
    std::string toAddress = ParseAddress(params[1]);
    uint32_t propertyId = ParsePropertyId(params[2]);
    int64_t uniqueTokenStart = params[3].get_int64(); //ParseAmount(params[3], false);
    int64_t uniqueTokenEnd =  params[4].get_int64(); //ParseAmount(params[4], false);
    ...
    // TODO: RequireUniqueOwner(fromAddress, propertyId, uniqueTokenStart, uniqueTokenEnd);
    // TODO: RequireRangeStartLessThanEnd(uniqueTokenStart, uniqueTokenEnd);
    RequireSaneReferenceAmount(referenceAmount);
    ...
    if (result != 0) {
        ...
        if (!autoCommit) {
            ...
            PendingAdd(txid, fromAddress, OMNI_TYPE_SEND_UNIQUE, propertyId, uniqueTokenAmount); // TODO: look at pending for unique tokens
            return txid.GetHex();
        }
    }
}
